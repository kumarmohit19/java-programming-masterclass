...
class Task1 extends Thread {
	...
	public void run() {
		// System.out.println(a+" "+beg);
		for (int j = start; j < end; j++) {
			Solution.threadArray[j] = count;
			count++;
		}
	}
}
...
class Task2 extends Thread {
	...
	public void run() {
		// System.out.println(a+" "+beg);
		for (int i = 0; i < Task3.beg; i++) {
			// System.out.println(Main.threadArray[i]);
			if (Solution.threadArray[i] <= 0 && i != 2) {
				Solution.threadArray[i] = count++;
			}
		}
	}
}
